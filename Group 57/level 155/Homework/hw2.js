...
function findBurnerIPs(setA, setB, setC) {
    let allIPs = new Map();

    for (let ip of setA) allIPs.set(ip, (allIPs.get(ip) || 0) + 1);
    for (let ip of setB) allIPs.set(ip, (allIPs.get(ip) || 0) + 1);
    for (let ip of setC) allIPs.set(ip, (allIPs.get(ip) || 0) + 1);

    let result = new Set();

    for (let [ip, count] of allIPs) {
        if (count === 1) {
            result.add(ip);
        }
    }
